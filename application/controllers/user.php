...
class User  extends CI_Controller {
	...
	public function addressDelete()
	{
		...
		// load model
		$this->load->model("usermodel");
		$resultado = $this->usermodel->addressDelete($idAddress);
		//echo $resultado;
		if(0==$resultado){
			$response["responseStatus"] = "Direccion no eliminada correctamente";		
		}
		else{
			$response["responseStatus"] = "Direccion  eliminada correctamente";
		}
		$this->output
		    ->set_content_type('application/json')
		    ->set_output(json_encode( $response ));
