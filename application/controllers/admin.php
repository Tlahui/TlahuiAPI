...
class Admin extends CI_Controller {
	...
	public function edit()
	{
		...
		//$newUser["id"] = $this->input->post("id");
		$User["correoElectronico"] = $this->input->post("correoElectronico");
		$User["usuario"] = $this->input->post("usuario");
		$User["password"] = $this->input->post("password");
		$User["nombre"] = $this->input->post("nombre");
		$User["fechaNacimiento"] = $this->input->post("fechaNacimiento");
		...
		// Si ocurre un error regresa 0 ... así que aquí checamos si la fecha
		// viene mal
		if( 0 === preg_match("/^\d{4}[\-\/\s]?((((0[13578])|(1[02]))[\-\/\s]?(([0-2][0-9])|(3[01])))|(((0[469])|(11))[\-\/\s]?(([0-2][0-9])|(30)))|(02[\-\/\s]?[0-2][0-9]))$/",$User["fechaNacimiento"]))
		{
			$response["responseStatus"]  = "Invalid Date Format. yyyy-mm-dd, yyyy mm dd, or yyyy/mm/dd Expected";
		}
		else
		{
			// Validamos el correo electrónico...
			if (!filter_var($User["correoElectronico"], FILTER_VALIDATE_EMAIL) === false)
			{
				// Validamos que aún no existan en la base de datos
				if($this->adminmodel->usernameIsUnique($User["usuario"]) && 
				 $this->adminmodel->emailIsUnique($User["correoElectronico"]))
				{	
					// Validamos que el password cumpla con ciertos criterios (al menos 6 posiciones, al menos una letra...)
					if(0 === preg_match("/^(?![0-9]{6,})[0-9a-zA-Z]{6,}$/",$User["password"]))
					{
						$response["responseStatus"] = "Password must contain at least one leter, minimum length 6, alphanumeric";
					}
					else
					{
						// Validamos el teléfono ...
						if(0 === preg_match("/^\D?(\d{3})\D?\D?(\d{3})\D?(\d{4})$/",$User["telefono"]))
						{
							$response["responseStatus"] = "Invalid phone number. Valid formats: 111-222-3333, or 111.222.3333, or (111) 222-3333, or 1112223333";
						}
						else
						{
							...
							// Le ponemos la "marca" de que éste usuario es de tipo Administrador
							$User["type"] = 1;							
							// ACTUALIZAMOS usuario a través de una llamada al modelo
							$resultado = $this->adminmodel->update($userID,$User);
							if($resultado == true)
							{
								$response["responseStatus"] = "OK";
							}
							...
							//$response["userID"] = $this->adminmodel->insertadmin($User);
							//$response["responseStatus"] = "OK";
						}
					}
				}
...
//		$response["responseStatus"] = "Not OK";
		...
		//llamar a la funcion del modelo para buscar ese id y obtener sus datos
